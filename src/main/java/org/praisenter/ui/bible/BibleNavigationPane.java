...
public final class BibleNavigationPane extends GridPane {
	...
	private final ObjectProperty<String> verse;
	...
	public BibleNavigationPane(GlobalContext context, BibleConfiguration configuration) {
		...
		this.verse = new SimpleObjectProperty<String>(String.valueOf(1));
		...
		Spinner<String> spnVerse = new Spinner<String>(1, Short.MAX_VALUE, 1, 1);
		...
		if (bible2 != null && bible2.getId() != triplet.getCurrent().getBible().getId()) {
			...
			if (matchingTriplet != null) {
				if (matchingTriplet.getCurrent() != null) {
					value.getVariant(TextVariant.SECONDARY).getReferenceVerses()
							.add(toReference(matchingTriplet.getCurrent()));
				}
				if (matchingTriplet.getPrevious() != null) {
					previous.getVariant(TextVariant.SECONDARY).getReferenceVerses()
							.add(toReference(matchingTriplet.getPrevious()));
				}
				if (matchingTriplet.getNext() != null) {
					next.getVariant(TextVariant.SECONDARY).getReferenceVerses()
							.add(toReference(matchingTriplet.getNext()));
				}
			}
		}
